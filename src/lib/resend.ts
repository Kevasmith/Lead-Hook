...
export async function sendEmail(to: string, subject: string, html: string, userId?: string | null) {
  const apiKey = await getResendKey(userId)
  if (!apiKey) throw new Error("Resend API key not configured. Add it in Settings.")
  const fromEmail = await getResendFromEmail(userId)
